...
Parameter.CPC.kp                        = 0.025;                          	% [s]       % proportional gain, brute-force-optimized (adjusted shakti 30.11 educated guess, JP),(0.03 works also, a bit mor agrassive fle)
...
Parameter.CPC.theta_min                 = deg2rad(0.5);                     % [rad]     % pitch angle in region 1-2.5, brute-force optimized - JP 15/122024
...
%% Torque Controller
Parameter.VSC.kp                        = 27928.661741;                       % [Nm/(rad/s)]  % proportional gain, first guess (adjusted shakti 12.12 task design script, fle)
Parameter.VSC.Ti                        = 2.196201;                           % [s]           % integral gain, first guess (adjusted shakti 12.12 task design script, fle)
% Parameter.VSC.k                         = 1.9;                              % [Nm/(rad/s)^2]% gain region 2, brute-force-optimized
Parameter.VSC.k                         = .5 * Parameter.General.rho * pi ... % [Nm/(rad/s)^2]% gain region 2, first guess: cp 0.48, lambda 8.75; 42.9 [NM/(rad/s)^2] brute-force-optimiert (fle)
...
Parameter.VSC.Delta_P                   = 4.95e6;                          	% [W]       % change of power at which over-speed limit should be reached, 4.9 MW brute-force-optimiert (fle)
